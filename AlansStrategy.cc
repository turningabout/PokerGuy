...
int Alan::AlansBettingStrategy(GameStats Stats) {
  ...
  if (Verbose > 0) cerr << "Current best score is: " << CurrentScore << endl;
...
int Alan::BotBettingStrategy(GameStats stats) {
  return CHECK;
  if (stats.GameStage == 0) return UtilNS::STARTINGCHIPS / 100 + 1;
  if (stats.GameStage - 1 < 0) return CHECK;
  return CHECK;
}
...
int Alan::Bot2BettingStrategy(GameStats stats) {
  return CHECK;
  if (stats.GameStage == 0) return UtilNS::STARTINGCHIPS / 5 + 1;
  if (stats.GameStage - 1 < 0) return CHECK;
  if (stats.GameStage == 1) return UtilNS::STARTINGCHIPS / 7 + 1;
        ...
        for (int d = s + 1; d < N; d++) {
          for (int f = d + 1; f < N; f++) {
            for (int g = f + 1; g < N; g++) {
              ...
              if (Verbose > 0) cerr << "Trying: ";
              for (int i = 0; i < 5; i++)
                if (Verbose > 0) cerr << Cards[i] << ' ';
              Score = HandValue(Cards, 5);
              if (Verbose > 0) cerr << "\t which gives: " << setprecision(14) << Score << endl;
